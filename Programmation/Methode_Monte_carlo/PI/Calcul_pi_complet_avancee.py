...
def Integration_MC(a:float, b:float, N: int, f: callable)-> float:
    ...
    l'estimation de l'erreur sur le calcul de l'integrale
    ...
    while i < N:
        ...
        if y_i > f_xi:
            ...
            gd1.plot(x_i,f_xi)                  # Affichage du point interieur
        else:
    ...
    integrale = A*somme_fx_i                                     # calcul de l'aire sous la courbe de f(x) sur [a,b]                             
...
if __name__ == "__main__" :
   for N in [1e3,1e4, 1e5]:
       print( f" Pour N = {N}\n Pi = {4*Integration_MC(0,1,N, f)} +/- {Integration_MC.erreur}")
